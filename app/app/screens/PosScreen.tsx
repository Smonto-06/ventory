...
export default function PosScreen() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', color: 'var(--text)', background: 'var(--bg)' }}>
      <header
        data-no-print="true"
        style={{ height: 60, flex: 'none', background: 'var(--surface)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 14, padding: '0 clamp(14px,3vw,26px)', position: 'sticky', top: 0, zIndex: 20 }}
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
          ...
          <span style={{ color: 'var(--muted)', fontSize: 13.5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>· {s.settings?.name}</span>
        </div>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ flex: 1, display: 'flex', flexWrap: 'wrap', alignItems: 'stretch' }}>
        <main style={{ flex: '1 1 460px', minWidth: 'min(100%,340px)', padding: 'clamp(14px,2.4vw,24px)', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <input
            value={query}
          ...
          {results.length > 0 ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(158px,1fr))', gap: 12, alignContent: 'start' }}>
              {results.map((p) => {
                ...
                return (
                  ...
                  >
                    <div style={{ height: 82, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', ...t.tileStyle }}>
        ...
        <aside style={{ flex: '0 1 380px', minWidth: 'min(100%,320px)', background: 'var(--surface)', borderLeft: '1px solid var(--border)', display: 'flex', flexDirection: 'column', position: 'sticky', top: 60, alignSelf: 'flex-start', maxHeight: 'calc(100vh - 60px)' }}>
                ...
                return (
                  <div key={it.productId} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '13px 0', borderBottom: '1px solid #EEF2F7' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, flex: 'none' }}>
                      <button onClick={() => s.changeQty(it.productId, -1)} style={{ width: 28, height: 28, borderRadius: 8, background: 'var(--bg)', color: 'var(--text)', fontWeight: 700, fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        −
                      </button>
                      <span style={{ minWidth: 22, textAlign: 'center', fontWeight: 700, fontSize: 14 }}>{it.qty}</span>
                      <button onClick={() => s.changeQty(it.productId, 1)} style={{ width: 28, height: 28, borderRadius: 8, background: '#EEF0FE', color: '#6366F1', fontWeight: 700, fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        +
                      </button>
                    </div>
                    <button
          ...
          <div style={{ flex: 'none', borderTop: '1px solid #EEF2F7', padding: '16px 20px', background: 'var(--input)' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
              <button
                onClick={() => (s.cart.length ? s.holdSale() : s.go('esperas'))}
